#!/usr/bin/env python3
"""Render a deterministic, self-contained Forge report from local JSON state."""
...
def _report_body(state: Mapping[str, Any]) -> str:
    title = _escape(state.get("title"), "Untitled Forge report")
    ...
    <div class="masthead-grid">
      <div>
        <p class="kicker">Forge / {html.escape(mode)} studio report</p>
        <h1>{title}</h1>
      </div>
      <div>
  ...
  <div class="studio">
    ...
    <main id="report-main" class="report" tabindex="-1">
      ...
      <aside class="feedback" aria-labelledby="feedback-title">
        ...
        <p>Draft only — this feedback grants no approval. Copy or download a local JSON proposal, then review it through Forge.</p>
        ...
        <div class="button-row">
          ...
          <a id="feedback-download-anchor" hidden download="forge-feedback.json"></a>
        </div>
...
def render_report(state: dict[str, Any]) -> str:
    """Return deterministic HTML for one Forge report-state mapping."""
    ...
    title = _text(state.get("title"), "Untitled Forge report")
    document_title = html.escape(f"{title} — Forge report", quote=True)
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Render a self-contained local Forge HTML report."
    )
    parser.add_argument("input", type=Path, help="Forge report-state JSON")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    try:
        ...
        return 2
    print(f"Rendered local Forge report: {args.output}")
    return 0
