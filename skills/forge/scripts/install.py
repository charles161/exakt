#!/usr/bin/env python3
"""Install the same self-contained Forge skill into supported harness homes."""
...
def install(host: str, root: Path, *, dry_run: bool = False) -> tuple[Path, str]:
    ...
    destination = root / "skills" / "forge" if host != "generic" else root / "forge"
    if destination.exists() or destination.is_symlink():
        raise InstallError(f"destination already exists: {destination}")
    command = root / "commands" / "forge.md" if host == "claude" else None
    if command is not None and (command.exists() or command.is_symlink()):
        raise InstallError(f"command already exists: {command}")
    if dry_run:
        invocation = "/forge <task>" if host == "claude" else "$forge <task>"
        return destination, invocation

    destination.parent.mkdir(parents=True, exist_ok=True)
    temporary = Path(
        tempfile.mkdtemp(prefix=".forge-install-", dir=destination.parent)
    )
    try:
        shutil.copytree(
            SKILL_ROOT,
    ...
    if host == "claude":
        source_command = PACKAGE_ROOT / ".claude" / "commands" / "forge.md"
        assert command is not None
        command.parent.mkdir(parents=True, exist_ok=True)
        command_text = source_command.read_text(encoding="utf-8").replace(
            "skills/forge/SKILL.md", str(destination / "SKILL.md")
        )
        command.write_text(command_text, encoding="utf-8")
        invocation = "/forge <task>"
    elif host == "codex":
        invocation = "$forge <task>"
    else:
        invocation = "Ask your harness to use forge/SKILL.md for <task>"
    return destination, invocation


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Install the portable Forge skill")
    ...
    try:
        ...
        print(f"forge install: {error}", file=sys.stderr)
        return 2
    label = "Would install" if args.dry_run else "Installed"
    print(f"FORGE  •  {label.upper()}")
