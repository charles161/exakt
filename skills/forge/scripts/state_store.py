"""Canonical bytes, trusted state, identities, and the Forge event journal.
...
def _load_contracts_module():
    ...
    spec = importlib.util.spec_from_file_location("_forge_state_contracts", path)
...
CANONICAL_JSON_VERSION = "forge-canonical-json-v1"
...
_PROBE_PREFIX = ".forge-capability-"
...
class CanonicalStateError(StateStoreError):
    """Input cannot be represented as Forge Canonical JSON v1."""
...
def canonical_json_bytes(value: Any) -> bytes:
    """Encode unframed Forge Canonical JSON v1 bytes.
...
def parse_json_bytes(data: bytes, *, require_canonical: bool = False) -> Any:
    ...
    if require_canonical and canonical_json_bytes(value) != data:
        raise CanonicalStateError("input is not Forge Canonical JSON v1")
    return value
...
def resolve_state_home(
    ...
    *,
    ...
    if "FORGE_STATE_HOME" in env:
        return _absolute_configured_path(
            env["FORGE_STATE_HOME"], variable="FORGE_STATE_HOME", home=home_path
        )
    ...
    if platform_value == "darwin":
        return home_path / "Library/Application Support/Forge"
    if is_windows:
        ...
        return local / "Forge"
    return home_path / ".local/state/forge"
...
def scope_id_for_repositories(repository_ids: Iterable[str]) -> str:
    ...
    if len(ordered) == 1:
        preimage = b"forge-scope-v1\n" + ordered[0].encode("ascii")
    else:
        preimage = b"forge-target-set-v1\n" + b"\n".join(
...
def _validate_tree_digest(vcs_kind: str, tree_digest: Any) -> None:
    ...
    if not isinstance(tree_digest, str) or pattern.fullmatch(tree_digest) is None:
        kind = "Git tree OID" if vcs_kind == "git" else "Forge SHA-256 content anchor"
...
def scrub_state_environment(environ: Mapping[str, str]) -> dict[str, str]:
    """Return a child environment without Forge canonical-state locations."""
    ...
    return {
        key: value
        for key, value in environ.items()
        if not key.casefold().startswith("forge_state_")
    }
