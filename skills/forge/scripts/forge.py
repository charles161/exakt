#!/usr/bin/env python3
"""Small portable controller for Forge's in-harness workflow and report UI."""
...
REPORT_VERSION = "forge-report-v1"
MODES = ("task", "product")
PHASES = (
    "intake",
    "recon",
...
class ForgeCliError(ValueError):
    pass
...
def title_from_request(request: str) -> str:
    ...
    if not first:
        raise ForgeCliError("request must not be empty")
...
def initial_state(request: str, mode: str, title: str | None = None) -> dict[str, Any]:
    if mode not in MODES:
        raise ForgeCliError(f"mode must be one of: {', '.join(MODES)}")
    ...
    return {
        ...
        "mode": mode,
        "summary": "Forge has captured the request. Reconnaissance and requirements are next.",
        "status": "draft",
        "phase": "intake",
        "updated_at": utc_now(),
...
def validate_state(state: Any) -> dict[str, Any]:
    if not isinstance(state, dict):
        raise ForgeCliError("report state must be a JSON object")
    required = {
        "schema_version",
        "title",
        "mode",
    ...
    if missing:
        raise ForgeCliError("report state is missing: " + ", ".join(missing))
    if state["schema_version"] != REPORT_VERSION:
        raise ForgeCliError("unsupported report schema version")
    if state["mode"] not in MODES or state["phase"] not in PHASES:
        raise ForgeCliError("report mode or phase is invalid")
    if state["status"] not in STATUSES:
        raise ForgeCliError("report status is invalid")
    for name in ("title", "summary", "updated_at"):
        if not isinstance(state[name], str):
            raise ForgeCliError(f"report {name} must be text")
    if not isinstance(state["brief"], dict) or not isinstance(
        state["architecture"], dict
    ):
        raise ForgeCliError("brief and architecture must be objects")
    for name in (
        "requirements",
        "acceptance_criteria",
        "tasks",
        ...
        if not _is_list(state[name]):
            raise ForgeCliError(f"report {name} must be a list")
    return state
...
def load_state(path: Path) -> dict[str, Any]:
    try:
        ...
        raise ForgeCliError(f"cannot read report state: {error}") from error
    return validate_state(state)


def write_state(path: Path, state: dict[str, Any], *, force: bool = False) -> None:
    if path.exists() and not force:
        raise ForgeCliError(f"refusing to overwrite existing state: {path}")
...
def render(state_path: Path, output: Path, *, force: bool = False) -> None:
    ...
    if output.exists() and not force:
        raise ForgeCliError(f"refusing to overwrite existing report: {output}")
    if not RENDERER.is_file():
        raise ForgeCliError("HTML renderer is not installed")
    ...
    if result.returncode != 0:
        ...
        raise ForgeCliError(f"renderer failed: {detail}")
...
def command_init(args: argparse.Namespace) -> int:
    ...
    print("FORGE  •  " + args.mode.upper() + "  •  INTAKE")
...
def command_status(args: argparse.Namespace) -> int:
    ...
    print(
        f"FORGE  •  {state['mode'].upper()}  •  {state['phase'].upper()}  •  {state['status'].upper()}"
    )
...
def command_verify(args: argparse.Namespace) -> int:
    ...
    if gaps:
        print("FORGE  •  NOT VERIFIED")
        for gap in gaps:
            print(f"- {gap}")
        return 2
    print("FORGE  •  VERIFIED")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="forge",
        description="Forge: spec, build, inspect, verify, and explain engineering work.",
    )
    ...
    init.add_argument("--output", default=".forge/forge-state.json")
