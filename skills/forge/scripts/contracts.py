"""Deterministic validation for Forge's portable JSON contract subset.

This module intentionally implements only the JSON Schema keywords used by
the bundled Forge v1 schemas.  Schema definitions are checked before use, and
...
class ContractError(ValueError):
    """A document does not satisfy a known Forge contract."""


class SchemaDefinitionError(ContractError):
    """A Forge schema uses malformed or unsupported contract semantics."""
...
def _check_schema_node(
    ...
    if not is_root and "$id" in schema:
        raise SchemaDefinitionError(
            f"{path} uses nested resource identifiers, which the Forge subset "
            "does not implement"
        )
    if not is_root and "$schema" in schema:
        raise SchemaDefinitionError(
            f"{path} uses nested schema dialects, which the Forge subset "
            "does not implement"
        )
    ...
    if "object" in schema_types:
        ...
        if "additionalProperties" in schema and not isinstance(
            ...
            raise SchemaDefinitionError(
                f"{path}.additionalProperties must be boolean in the Forge subset"
            )
...
class ContractRegistry:
    """Load, verify, select, and apply Forge schemas from one directory."""
