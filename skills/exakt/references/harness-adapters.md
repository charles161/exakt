...
|---|---|---|---|
| Invoke | `$exakt <task>` | `/exakt <task>` | Load `SKILL.md` with the task |
...
| Report | `exakt.py render` and a local link | `exakt.py render` and a local link | `exakt.py render` and its path |
...
Use the installed `skills/exakt/scripts/exakt.py` as the state/report entry point. Check `python3 skills/exakt/scripts/exakt.py --help`, then invoke its `render` command with the current work item. Do not handcraft or silently patch generated HTML. Keep a terminal handoff if rendering is unavailable or fails.
